...
if [[ "$old_process" == "1" ]]; then
    if test -f "/etc/systemd/system/nkn-commercial.service"; then
...
Download() {
    ...
    if test -f "config.json"; then
        ...
        if test -f "wallet.json"; then
            start_shell="${WORKDIR}/npool --appkey ${app_key} --wallet ${WORKDIR}/wallet.json --password-file ${WORKDIR}/wallet.pswd"
        else
            start_shell="${WORKDIR}/npool --appkey ${app_key}"
        fi
    else
        wget -c -t 5 --quiet "https://download.npool.io/linux-${arch_type}.tar.gz" && tar zxvf "linux-${arch_type}.tar.gz"
        start_shell="${WORKDIR}/linux-${arch_type}/npool --appkey ${app_key}"
    fi
    if [[ "$pruning_mode" == "no-pruning" ]]; then
