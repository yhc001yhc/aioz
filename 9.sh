...
if [ ! -f "$IMAGE_FILE" ]; then
    sudo dd if=/dev/zero of=$IMAGE_FILE bs=1M count=20480 # 使用较小的block size以避免内存耗尽问题
fi
...
# 运行带有存储限制的 Docker 容器
docker run --name station --detach --env FIL_WALLET_ADDRESS=0xea9d5983c9391ec3e40870e7b4c8051756a83a82 --storage-opt size=700M ghcr.io/filecoin-station/core
docker run -d --name watchtower --restart=always --storage-opt size=50M -v /var/run/docker.sock:/var/run/docker.sock containrrr/watchtower --interval 36000 --cleanup
